...
namespace Sprockets.Lucene.Net.Linq {
    ...
    public static class LuceneMethods {
        private const string UnreachableCode =
                "Unreachable code. This method should have been translated within a LINQ expression and not directly invoked."
            ;
        ...
        /// </summary>
        public static IQueryable<T> CaptureStatistics<T>(this IQueryable<T> source,
            Action<LuceneQueryStatistics> callback) {
        ...
        /// </summary>
        internal static IQueryable<T> BoostInternal<T>(this IQueryable<T> source,
            Expression<Func<T, float>> boostFunction) {
            return source.Provider.CreateQuery<T>(
                Expression.Call(((MethodInfo) MethodBase.GetCurrentMethod()).MakeGenericMethod(typeof(T)),
                    source.Expression,
                    boostFunction));
        ...
        internal static IQueryable<T> TrackRetrievedDocuments<T>(this IQueryable<T> source,
            IRetrievedDocumentTracker<T> tracker) {
            return source.Provider.CreateQuery<T>(
                Expression.Call(((MethodInfo) MethodBase.GetCurrentMethod()).MakeGenericMethod(typeof(T)),
                    source.Expression,
