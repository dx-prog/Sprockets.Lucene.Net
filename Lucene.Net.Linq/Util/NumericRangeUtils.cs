...
namespace Sprockets.Lucene.Net.Linq.Util {
    internal static class NumericRangeUtils {
        internal static Query CreateNumericRangeQuery(string fieldName,
            ...
            if (lowerBound.GetType() != upperBound.GetType())
                throw new ArgumentException("Cannot compare different value types " + lowerBound.GetType() + " and " +
                                            upperBound.GetType());
        ...
        internal static NumericField ForceDisableOmitNorms(this NumericField field) {
            ...
            if (fieldInfo == null)
                throw new InvalidOperationException(string.Format(
                    "Type {0} does not have a non-public field named {1}.",
                    typeof(AbstractField),
                    fieldName));
