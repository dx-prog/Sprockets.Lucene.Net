...
namespace Sprockets.Lucene.Net.Linq.Transformation.TreeVisitors {
    internal class AggressiveSubQueryFromClauseFlattener : SubQueryFromClauseFlattener {
        protected override void CheckFlattenable(QueryModel subQueryModel) {
            ...
            if (first != null)
                throw new NotSupportedException(first.GetType() +
                                                " is not supported in sub-queries. Sub-queries may only use SequenceTypePreservingResultOperatorBase subclasses.");
        }

        protected override void FlattenSubQuery(SubQueryExpression subQueryExpression,
        ...
        protected virtual void MoveResultOperatorsToParent(QueryModel queryModel, QueryModel subQueryModel) {
            foreach (var resultOperator in subQueryModel.ResultOperators
                .OfType<SequenceTypePreservingResultOperatorBase>()
                .Reverse()
                .ToList()) {
