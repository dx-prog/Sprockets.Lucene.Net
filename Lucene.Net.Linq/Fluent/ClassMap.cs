...
namespace Sprockets.Lucene.Net.Linq.Fluent {
    ...
    public class ClassMap<T> {
        private readonly IDictionary<string, string> documentKeys =
        ...
        /// </param>
        /// <returns>
        ///     A <see cref="PropertyMap{T}" /> that allows further customizatoin of
        ...
        public IDocumentMapper<T> ToDocumentMapper() {
            ...
            foreach (var kv in documentKeys) {
                if (ReferenceEquals(kv.Value, null))
                    throw new InvalidOperationException("Must specify non-null document key value for key field " +
                                                        kv.Key);
