...
namespace Sprockets.Lucene.Net.Linq.Translation.TreeVisitors {
    internal class QueryBuildingExpressionTreeVisitor : LuceneExpressionTreeVisitor {
        ...
        protected override Expression VisitBinary(BinaryExpression expression) {
            switch (expression.NodeType) {
                ...
                default:
                    throw new NotSupportedException("BinaryExpression of type " + expression.NodeType +
                                                    " is not supported.");
            }
        }
        ...
        protected override Expression VisitLuceneQueryPredicateExpression(LuceneQueryPredicateExpression expression) {
            ...
            if (expression.QueryType == QueryType.GreaterThan || expression.QueryType == QueryType.GreaterThanOrEqual)
                query = CreateRangeQuery(mapping,
        ...
        protected override Expression VisitLuceneRangeQueryExpression(LuceneRangeQueryExpression expression) {
            ...
            var query = CreateRangeQuery(mapping,
                expression.LowerQueryType,
                expression.Lower,
                expression.UpperQueryType,
                expression.Upper);
        ...
        private void AddMultiFieldQuery(LuceneQueryPredicateExpression expression) {
            ...
            query.Add(new BooleanClause(fieldMappingInfoProvider.CreateMultiFieldQuery(GetPattern(expression, null)),
                expression.Occur));
            queries.Push(query);
        }

        private Query CreateRangeQuery(IFieldMappingInfo mapping,
            QueryType lowerQueryType,
            Expression lowerBoundExpression,
            ...
            var lowerRange = lowerQueryType == QueryType.LessThan || lowerQueryType == QueryType.GreaterThan
                ? RangeType.Exclusive
                : RangeType.Inclusive;
            var upperRange = upperQueryType == QueryType.LessThan || upperQueryType == QueryType.GreaterThan
                ? RangeType.Exclusive
                : RangeType.Inclusive;
        ...
        private string EvaluateExpressionToString(LuceneQueryPredicateExpression expression,
            IFieldMappingInfo mapping) {
