...
namespace Sprockets.Lucene.Net.Linq.Search.Function {
    internal class DelegatingCustomScoreQuery<T> : CustomScoreQuery {
        ...
        public DelegatingCustomScoreQuery(Query subQuery,
            Func<Document, T> convertFunction,
        ...
        private class DelegatingScoreProvider : CustomScoreProvider {
            ...
            public DelegatingScoreProvider(IndexReader reader,
                Func<Document, T> convertFunction,
                Func<T, float> scoreFunction)
                : base(reader) {
                this.convertFunction = convertFunction;
