...
using DateTimeConverter = Sprockets.Lucene.Net.Linq.Converters.DateTimeConverter;
    ...
    internal class FieldMappingInfoBuilder {
        ...
        internal static TypeConverter GetConverter(PropertyInfo p, Type type, FieldAttribute metadata) {
            ...
            if (converter == null || !converter.CanConvertFrom(typeof(string)))
                throw new NotSupportedException("Property " + p.Name + " of type " + p.PropertyType +
                                                " cannot be converted from " + typeof(string));
        ...
        internal static Analyzer CreateAnalyzer(Type analyzer, Version version) {
            if (!typeof(Analyzer).IsAssignableFrom(analyzer))
                throw new InvalidOperationException("The type " + analyzer + " does not inherit from " +
                                                    typeof(Analyzer));
            ...
            throw new InvalidOperationException("The analyzer type " + analyzer +
                                                " must have a public default constructor or public constructor that accepts " +
                                                typeof(Version));
        }

        private static ReflectionFieldMapper<T> BuildPrimitive<T>(PropertyInfo p,
            Type type,
            FieldAttribute metadata,
            Version version,
            ...
            var defaultParserOperator = metadata != null ? metadata.DefaultParserOperator : QueryParser.Operator.OR;
            ...
            return new ReflectionFieldMapper<T>(p,
                store,
                index,
                termVectorMode,
                converter,
