...
using Sprockets.Lucene.Net.Linq.ScalarResultHandlers;
using Sprockets.Lucene.Net.Linq.Search.Function;
using Sprockets.Lucene.Net.Linq.Transformation;
using Sprockets.Lucene.Net.Linq.Translation;
...
namespace Sprockets.Lucene.Net.Linq {
    internal abstract class LuceneQueryExecutorBase<TDocument> : IQueryExecutor, IFieldMappingInfoProvider {
        ...
        public T ExecuteScalar<T>(QueryModel queryModel) {
            ...
            using (searcherHandle) {
                ...
                var executionContext =
                    new QueryExecutionContext(searcher, luceneQueryModel.Query, luceneQueryModel.Filter);
                TopFieldDocs hits;
                ...
                if (maxResults > 0) {
                    ...
                    hits = searcher.Search(executionContext.Query,
                ...
                var handler =
                    ScalarResultHandlerRegistry.Instance.GetItem(luceneQueryModel.ResultSetOperator.GetType());
                ...
                RaiseStatisticsCallback(luceneQueryModel,
                    executionContext,
        ...
        public IEnumerable<T> ExecuteCollection<T>(QueryModel queryModel) {
            ...
            using (searcherHandle) {
                ...
                if (scoreFunction != null)
                    query = new DelegatingCustomScoreQuery<TDocument>(query,
                        ConvertDocumentForCustomBoost,
                        scoreFunction);
                ...
                var hits = searcher.Search(executionContext.Query,
                    executionContext.Filter,
                    maxResults + skipResults,
                    luceneQueryModel.Sort);
                ...
                foreach (var p in EnumerateHits(hits,
                    executionContext,
                    searcher,
                    tracker,
                    itemHolder,
        ...
        protected virtual Expression<Func<TDocument, T>> GetProjector<T>(QueryModel queryModel) {
            return Expression.Lambda<Func<TDocument, T>>(queryModel.SelectClause.Selector,
                Expression.Parameter(typeof(TDocument)));
        }
        ...
        private void RaiseStatisticsCallback(LuceneQueryModel luceneQueryModel,
        ...
        private IEnumerable<T> EnumerateHits<T>(TopDocs hits,
            QueryExecutionContext executionContext,
            Searchable searcher,
            IRetrievedDocumentTracker<TDocument> tracker,
            ItemHolder itemHolder,
            int skipResults,
            Func<TDocument, T> projector) {
